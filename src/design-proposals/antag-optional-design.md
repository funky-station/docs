...
## Background
This document is built around the current state of the game, where two things are true:
...
Beyond this, the broader design discussion around Funky has repeatedly circled the same tension this document tries to resolve directly: proposals tend to frequently get pushback on the basis of how it would interact with antagonists. 
...
This document is the philosophy Funky will sit on, the thing to point to when arguing that a system change doesn't need to justify itself in terms of antagonist balance.
...
Anything currently justified purely by "but antags" should be re-evaluated by its own merits under this philosophy. If it doesn't hold up without an antag to justify it, removing it is the expected outcome, in the same spirit as prior removals like IPCs and Atmos Tech.

**What replaces antag-driven engagement:**
- Union labor disputes and strikes (pay, conditions, benefits).
- Scrip-driven friction: bribery, embezzlement, wage disputes.
...
**Current antagonist-adjacent content:** Antags may still exist. Their defining trait, however, per this document's philosophy, is that they must be able to accomplish nothing, or succeed completely silently, without the round being worse for it.

## Game Design Rationale

The core principle this aligns with is that a round's engagement should come from the systems and people actually in the round, not from some switch that was flipped at round start. 
...
Our corporate-dystopia theming supports this directly. Security isn't some neutral peacekeeping force, it's a corporate boot, and Command cannot be good people in this setting; holding a Command position means being complicit in NanoTrasen's exploitation of the crew. 
...
To be explicit about what this document is not arguing: it is not asking for less violence, less death, or a quieter, lower-stakes server. 
...
Players should not treat the absence of escalation rules as license for arbitrary violence disconnected from character or circumstance; the enforcement mechanism for that is IC consequence (round-removal, meaningful medical injuries, long arrest period and trial, etc.).
...
It depends on Unions, Scrip, and Department Budgets being implemented and balanced, but otherwise this document mostly servers as a server philosophy piece.